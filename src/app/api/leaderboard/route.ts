...
export async function GET(req: NextRequest) {
  ...
  // Show first names only for privacy across families
  ...
  // Cap to top 50, but include both all-time and weekly leaders so newcomers
  // who rank high this week aren't excluded by a low lifetime total
  const topByTotal = [...entries].sort((a, b) => b.totalPoints - a.totalPoints).slice(0, 50);
  const topByWeekly = [...entries].sort((a, b) => b.weeklyPoints - a.weeklyPoints).slice(0, 50);
  const keep = new Map<string, typeof entries[number]>();
  for (const e of [...topByTotal, ...topByWeekly]) keep.set(e.id, e);

  const leaderboard = [...keep.values()].sort((a, b) => b.totalPoints - a.totalPoints);
